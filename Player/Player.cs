    ...
    public sealed partial class Player : IDisposable
    {
        ...
        void HandleLogin(byte[] message)
        {
            ...
            if (playerDb.Rows.Count == 0)
            {
                ...
                MySQL.executeQuery("INSERT INTO Players (Name, IP, FirstLogin, LastLogin, totalLogin, Title, totalDeaths, Money, totalBlocks, totalKicked, TimeSpent, sponges, lives)" +
                    "VALUES ('" + name + "', '" + ip + "', '" + firstLogin.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', " + totalLogins +
                    ", '" + prefix + "', " + overallDeath + ", " + money + ", " + loginBlocks + ", " + totalKicked + ", '" + time + "', '" + spongesLeft + "', '" + lives + "')");
            ...
            else
            {
                ...
                spongesLeft = int.Parse(playerDb.Rows[0]["sponges"].ToString());
                lives = int.Parse(playerDb.Rows[0]["lives"].ToString());
            ...
            {
                Command.all.Find("lavasurvival").Use(this, "go");
            }
        }
        ...
        public void manualChange(ushort x, ushort y, ushort z, byte action, byte type)
        {
            ...
            if (Server.lava.RoundActive() && lives <= 0)
            {
                SendMessage(c.lime + "You need to purchase a life at the /store or wait for the round to finish!");
                SendBlockchange(x, y, z, b); return;
            }

            string Username;
            bool Deleted;
            List<Level.BlockPos> inCache = this.level.blockCache.FindAll(bPz => bPz.x == x && bPz.y == y && bPz.z == z);
            ...
            for (int i = 0; i < inCache.Count; i++)
            {
                ...
                if (this.group.Permission < LevelPermission.AdvBuilder)
                {
                    if (!Deleted && Username.Trim() != this.name && bPz.type != Block.air)
                    {
                        Player.SendMessage(this, c.lime + "You may " + c.red + "NOT " + c.lime + "destroy other players blocks. [Owned by " + c.red + Username.Trim() + c.lime + "]");
                        SendBlockchange(x, y, z, b); return;
                    }
                    else { }
                }
        ...
        void HandleInput(object m)
        {
            ...
            else
            {
                ...
                if (NoClipcount == 0 && CheckIfInsideBlock(this))
                {
                    NoClipcount = 1;
                    if (countOfTimes >= 2)
                        SendMessage(c.aqua + "You will not get any " + Server.moneys + " for staying here!");
                    else
                        countOfTimes++;
                }
                else if (!CheckIfInsideBlock(this))
                {
                    NoClipcount = 0;
                }
        ...
        public void HandleDeath(byte b, string customMessage = "", bool explode = false)
        {
            ...
            if (PlayerDeath != null)
                PlayerDeath(this, b);
            if (lastDeath.AddSeconds(7) < DateTime.Now && Server.lava.RoundActive() == true && lives > 0)
            {
                ...
                if (level.Killer && !invincible)
                {
                    ...
                    switch (b)
                    {
                        case Block.tntexplosion: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " &cblew into pieces.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.deathair: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " walked into &cnerve gas and suffocated.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.deathwater:
                        case Block.activedeathwater: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " stepped in &dcold water and froze.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.deathlava:
                        case Block.activedeathlava: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " stood in &cmagma and melted.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.magma: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was hit by &cflowing magma and melted.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.geyser: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was hit by &cboiling water and melted.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.birdkill: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was hit by a &cphoenix and burnt.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.train: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was hit by a &ctrain.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.fishshark: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was eaten by a &cshark.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.fire: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " burnt to a &ccrisp.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.rockethead: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was &cin a fiery explosion.", false); level.MakeExplosion(x, y, z, 0); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.zombiebody: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " died due to lack of &5brain.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        ...
                        case Block.air: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " hit the floor &chard.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.water: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " &cdrowned.", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.Zero: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was &cterminated", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.fishlavashark: GlobalChatLevel(this, this.color + this.prefix + this.name + Server.DefaultColor + " was eaten by a ... LAVA SHARK?!", false); this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!"); break;
                        case Block.rock:
                            ...
                            this.SendMessage(c.lime + "You have " + c.red + lives + c.lime + " lives left!");
                            break;
                        case Block.stone:
                            if (explode) level.MakeExplosion(x, y, z, 1);
        ...
        public void HandleCommand(string cmd, string message)
        {
            try
            {
                ...
                else if (group.Permission >= LevelPermission.Operator && cmd.ToLower().Equals("normal_sponge"))
                {
                    HandleCommand("mode", "sponge");
                    return;
                }
                else if (Block.Byte(cmd.ToLower()) != Block.Zero)
                {
                    if (group.Permission >= LevelPermission.Operator && cmd.ToLower().Equals("sponge"))
                    {
                        HandleCommand("mode", "lava_sponge");
                        return;
                    }
                    else if (group.Permission <= LevelPermission.Operator && cmd.ToLower().Equals("lava_sponge"))
                    {
                        HandleCommand("mode", "sponge");
                        return;
                    }
        ...
        public static void GlobalChat(Player from, string message, bool showname)
        {
            ...
            try
            {
                if (Server.lava.HasPlayer(from) && Server.lava.HasVote(message.ToLower()))
                {
                    ...
                    {
                        SendMessage(from, "Your vote for &5" + message.ToLower() + Server.DefaultColor + " has been placed. Thanks!");
                        Server.lava.map.ChatLevelOps(from.name + " voted for &5" + message.ToLower() + Server.DefaultColor + ".");
                        return;
                    }
                    else
                    {
                ...
                else if(Server.lava.voteActive)
                {
                    if (!from.voice)
                    {
                        from.SendMessage("You may not speak while voting is active!");
                        return;
                    }
                }
            }
