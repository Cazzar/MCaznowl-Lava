...
public static byte maxGuests = 10;
  ...
  //         Server.ErrorLog(e);
                ...
                DataTable sponges = MySQL.fillData("SHOW COLUMNS FROM Players WHERE `Field`='sponges'");
                if (sponges.Rows.Count == 0)
                    MySQL.executeQuery("ALTER TABLE Players ADD COLUMN sponges BIGINT AFTER totalCuboided");
                sponges.Dispose();
                DataTable lives = MySQL.fillData("SHOW COLUMNS FROM Players WHERE `Field`='lives'");
                if (lives.Rows.Count == 0)
                    MySQL.executeQuery("ALTER TABLE Players ADD COLUMN lives BIGINT AFTER sponges");
                lives.Dispose();
            }
            ...
            ml.Queue(delegate
            {
                ...
                {
                    s.Log("Done.");
                    Command.all.Find("lavasurvival").Use(null, "start dragon");
                }
                else
                {
