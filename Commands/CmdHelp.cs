            ...
            try
            {
                ...
                switch (message)
                {
                    case "":
                        ...
                        {
                            Player.SendMessage(p, "Use &b/help lava" + Server.DefaultColor + " for a list of lava commands.");
                    ...
                    case "lava":
                        ...
                        foreach (Command comm in Command.all.commands)
                        {
                            if (p == null || p.group.commands.All().Contains(comm))
                            {
                                if (comm.type.Contains("lava")) message += ", " + getColor(comm.name) + comm.name;
                            }
                        }

                        if (message == "") { Player.SendMessage(p, "No commands of this type are available to you."); break; }
                        Player.SendMessage(p, "Lava commands you may use:");
                        Player.SendMessage(p, message.Remove(0, 2) + ", " + Group.Find("op").color + "sponge, normal_sponge, op_air, op_lair.");
                        break;
